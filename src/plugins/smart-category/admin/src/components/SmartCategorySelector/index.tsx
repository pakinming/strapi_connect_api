...
const SmartCategorySelector = ({ value, onChange, name, attribute, disabled, error, required }) => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      try {
        const { data } = await get('/content-manager/collection-types/api::category.category?fields[0]=name&fields[1]=documentId');
  ...
  useEffect(() => {
    const fetchChildren = async () => {
      ...
      try {
        ...
        const { data } = await get(`/content-manager/collection-types/api::category-child.category-child?filters[category][documentId][$eq]=${selectedCategory}&fields[0]=name&fields[1]=documentId`);
        const items = data.results || data.data || [];
  ...
  return (
    <Box>
      <Box paddingBottom={4}>
        <Field name={name} id={name} error={error} hint={attribute.hint}>
          <Flex direction="column" alignItems="stretch" gap={1}>
             <FieldLabel required={required}>Smart Category & Children (Field)</FieldLabel>
